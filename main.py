...
if raw_input:
    ...
    if data:
        ...
        fig.add_annotation(x=r_blue*0.7, y=r_blue*0.7, text=f"기본: Ø{tol:.3f}", showarrow=False, font=dict(color="RoyalBlue"))
        if max_total_tol > tol:
            fig.add_annotation(x=display_r_red*0.7, y=-display_r_red*0.7, text=f"최종(MMC): Ø{max_total_tol:.3f}", showarrow=False, font=dict(color="Red"))
        ...
        for res in ["✅ OK", "❌ NG"]:
            ...
            if not visible.empty:
                fig.add_trace(go.Scatter(x=visible['편차_X'], y=visible['편차_Y'], mode='markers+text', name=res,
                                         text=visible['측정포인트'], marker=dict(size=10, color="#2ecc71" if res=="✅ OK" else "#e74c3c")))

        fig.update_layout(width=700, height=700, xaxis=dict(range=[-view_limit, view_limit]), yaxis=dict(range=[-view_limit, view_limit]))
        st.plotly_chart(fig)
        
        # --- 🚨 [추가] 규격 이탈 시료 집중 관리 섹션 ---
        st.divider()
        ng_list = df[df['판정'] == "❌ NG"]
        
        if not ng_list.empty:
            st.error(f"🚨 **규격 이탈(NG) 시료 안내 (총 {len(ng_list)}건)**")
            # 실무자가 보기 편하게 리스트업
            for idx, row in ng_list.iterrows():
                # 어느 정도 벗어났는지(초과량) 계산
                excess = row['위치도'] - row['최종공차']
                st.write(f"- **{row['측정포인트']}**: 위치도 **{row['위치도']:.3f}** (허용규격 Ø{row['최종공차']:.3f} 대비 **{excess:.3f} 초과**)")
        else:
            st.success("✅ **모든 시료가 최종 합격(MMC 적용) 범위 내에 있습니다.**")
