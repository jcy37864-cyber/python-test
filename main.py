...
st.set_page_config(page_title="Quality Hybrid v2.4", layout="wide")
...
def run_analysis():
    ...
    with st.sidebar:
        st.header("⚙️ 설정")
        mode = st.radio("유형 선택", ["유형 B (가로형)", "유형 A (3줄 세트)"])
        sc = st.number_input("샘플 수", min_value=1, value=4)
        tol = st.number_input("기본 공차(Ø)", value=0.350, format="%.3f")
        mmc_ref = st.number_input("기준치(MMC/B:0.35)", value=0.060 if mode == "유형 A (3줄 세트)" else 0.350, format="%.3f")
        view_mode = st.radio("범위", ["자동", "수동"], horizontal=True)
        v_limit_input = st.slider("줌", 0.1, 5.0, 0.5) if view_mode == "수동" else 0.5
        ...
        try:
            ...
            if mode == "유형 B (가로형)":
                ...
                for i in range(0, len(lines), 4):
                    ...
                    nx = float(re.sub(r'[^0-9\.\-]', '', lines[i+2][0]))
                    ny = float(re.sub(r'[^0-9\.\-]', '', lines[i+3][0]))
                ...
                rows = [[float(v) for v in re.findall(r'[-+]?\d*\.\d+|\d+', l)] for l in lines]
                rows = [[v if abs(v) < 150 else v % 100 for v in r] for r in rows if r]
            ...
            df['BONUS'] = (df['DIA'] - mmc_ref).clip(lower=0).round(4)
            df['LIMIT'] = (tol + df['BONUS']).round(4)
            df['RES'] = np.where(df['POS'] <= df['LIMIT'], "✅ OK", "❌ NG")

            mx_lim = df['LIMIT'].max()
            v_l = round((mx_lim / 2) * 1.5, 2) if view_mode == "자동" else v_limit_input
            ...
            if not df[df['RES'] == "❌ NG"].empty:
                ...
                for _, r in df[df['RES'] == "❌ NG"].iterrows():
                    st.write(f"• {r['ID']}: {r['POS']} (허용: {r['LIMIT']})")
